...
with $$\sigma$$ being the sigmoid function. If we consider the Bernoullo likelihood function, then the likelihood for a datapoint $$D=\{(x_i, y_i)\}$$ (using $$i$$ as the single subscript to differentiate the datapoint from the variables $$x$$ and $$y$$) is
...
As we are in log-space, $$\ell_{\beta_{t-1}}(y_t,x_t)$$ can be absorbed into the normalizing constant, and combined with the assumption on $$\beta | D_{t-1}$$, we have
...
In order to derive the update, we must simply this expression via completing the matrix square to become of the form
...
We will also rewrite $$P_t$$ by the [Sherman-Morrison](https://en.wikipedia.org/wiki/Sherman–Morrison_formula):
...
where $$W_t(\beta) := \text{diag}( \sigma(X_t \beta) \odot (1 - \sigma(X_t \beta))$$ 
...
However, this requires you to calculate a pesky inverse matrix without a clean formula, i.e., the term $$( B^{-1} + C^T A^{-1} C)^{-1}$$. However, a simpler approach is to use this formula for $$(A+B)^{-1}$$ which comes from [this paper](https://www.jstor.org/stable/2690437), namely when $$A$$ and $$A+B$$ are invertible and the trace of $$BA^{-1}\neq 1$$,
